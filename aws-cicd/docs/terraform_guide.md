...
パイプラインが正常に動作するために、以下の3ファイルをアプリリポジトリのルートに配置する。

| ファイル | 配置元 | 説明 |
|---|---|---|
| `buildspec.yml` | `aws-cicd/buildspec.yml` をコピー | CodeBuild のビルド定義 |
| `appspec.yaml` | `aws-cicd/appspec.yaml` をコピー | CodeDeploy の動作定義 |
| `taskdef.json` | `aws-cicd/taskdef.json` をコピーして編集 | ECS タスク定義テンプレート |

`buildspec.yml` と `appspec.yaml` はコピーするだけで使用できる。  
`taskdef.json` は以下のプレースホルダーを書き換えること。

| プレースホルダー | 書き換え内容 | 確認場所 |
|---|---|---|
| `<PROJECT_NAME>` | `project_name` の値（例: `my-app`） | `terraform.tfvars` の `project_name` |
| `<ACCOUNT_ID>` | AWS アカウント ID（12桁の数字） | `aws sts get-caller-identity` の `Account` |
| `<REGION>` | デプロイ先リージョン（例: `ap-northeast-1`） | `terraform.tfvars` の `aws_region` |
...
```
your-app-repo/
├── buildspec.yml     ← aws-cicd/buildspec.yml をコピー
├── appspec.yaml      ← aws-cicd/appspec.yaml をコピー
├── taskdef.json      ← aws-cicd/taskdef.json をコピーしてプレースホルダーを書き換え
