...
## 8. aws-app のデプロイ（ローカル操作 / 新規ECS構築時のみ）

ECS・ALB・CodeDeploy を構築する。`aws-app` リポジトリに移動して以下を実行する。
...
## 9. aws-app デプロイ後の再設定（ローカル操作 / 新規ECS構築時のみ）
