...
| 決定 | 理由 |
|---|---|
| ECRはCI/CD側に含める | アプリ側でECSを使用することを想定しているため。ECSを使用しない場合はIaCからECRを除外しても問題ない |
| CodeDeployのDeploymentGroupはアプリ側に含める | ALB・ECSリソースへの参照が必要なため、アプリインフラと一体で管理すべき |
| CloudFormationはスタックを5本に分割 | ECR → ソース選択（CodeCommit版 or GitHub版）→ IAM → パイプライン の順に依存関係があるため分割。IAMを分離することでロール変更がパイプラインスタックに影響しない。ソース種別をスタック差し替えで切り替え可能にする |
