...
| リポジトリ | 管理内容 |
|---|---|
| `aws-cicd`（本リポジトリ） | CI/CDパイプライン（CodePipeline / CodeBuild / CodeDeploy / ECR / S3 / IAM） |
| `aws-app` | アプリ基盤（ECS / ALB / CodeDeploy Deployment Group） |
...
|---|---|---|---|
| 本リポジトリのみデプロイ済み | ✅ | ✅（ECRへのpushまで） | ❌（ECSが存在しないため失敗） |
| `aws-app` もデプロイ済み | ✅ | ✅ | ✅ |

**構築順序：** `aws-app` を先にデプロイし、出力されるリソース名を本リポジトリのパラメータとして渡す。
...
| AWS CodeDeploy Application | デプロイアプリケーション定義（Deployment Groupは`aws-app`側） |
...
| CodeDeploy Deployment Group | ALB・ECSへの参照が必要なため`aws-app`と一体で管理 |
...
> **使い方**: `02-source-codecommit.yaml` と `02-source-github.yaml` はどちらか一方のみデプロイする。
```
...
- `aws-app` がデプロイ済みであること（Deploy Stageを動作させる場合）
