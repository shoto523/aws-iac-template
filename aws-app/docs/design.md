...
> **ECS タスクのパブリックサブネット配置について**  
> NAT Gateway のコスト削減のため、ALB と ECS タスクを同じパブリックサブネットに配置し、ECS タスクにパブリック IP を付与する構成です。  
> ECS タスクにパブリック IP が付きますが、Security Group で ALB からのトラフィックのみを許可することでセキュリティを担保します。
...
| ECS タスク用 SG | ALB 用 SG → `container_port` のみ | すべて許可（ECR pull・ログ送信） |
...
| ネットワーク | `public_subnet_ids` | ALB・ECSタスクを配置するパブリックサブネットID（異なるAZに2つ以上） | `subnet-xxx,subnet-yyy` |
...
アプリリポジトリの `buildspec.yml` 内にある `taskdef.json` 生成コマンドの `environment` フィールドに追記します。
...
| 設定場所 | 管理者 | 参照 |
|---|---|---|
| アプリリポジトリの `buildspec.yml`（taskdef.json 生成部分） | アプリ開発者 | [aws-cicd/docs/buildspec_design.md](../../aws-cicd/docs/buildspec_design.md) |

> **シークレット（パスワード・APIキー）は直書きしないこと。**  
