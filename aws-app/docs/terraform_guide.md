...
| 前提条件 | 確認方法 |
|---|---|
| `aws-cicd` のデプロイ完了（ECR作成済み） | `terraform output ecr_repository_url`（aws-cicd/terraform/ で実行） |
| VPC / Subnet / Security Group が構築済み | AWSマネジメントコンソール or `aws ec2 describe-vpcs` |
| AWS CLI 認証情報設定済み | `aws sts get-caller-identity` |
...
# ネットワーク情報（事前に用意したVPC/Subnetの値を設定）
vpc_id                = "vpc-xxxxxxxx"
...
> `ecr_repository_url` は `aws-cicd/terraform/` ディレクトリで `terraform output ecr_repository_url` を実行して取得する。
...
2. aws-app を terraform apply（本ガイド）
      ↓ ecs_cluster_name 等を取得
3. aws-cicd を terraform apply（再実行）
      ↓ Deploy Stage が有効化
4. ソースリポジトリを接続
```
...
または AWS マネジメントコンソールで以下を確認する：

- ECS サービスのタスクが `RUNNING` 状態になっていること
- ALB のターゲットグループ（Blue）のヘルスチェックが `healthy` になっていること
