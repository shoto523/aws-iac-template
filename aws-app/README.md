...
| パブリックサブネット（**異なるAZに2つ以上・必須**） | ALB・ECS タスクの配置先 | ALB の仕様により異なるAZに最低2つ必要。ECS タスクも同じサブネットに配置 |
| Internet Gateway | VPC からインターネットへの出入口 | パブリックサブネットのルートテーブルにアタッチ |
| ALB 用セキュリティグループ | ALB へのアクセス制御 | インバウンド: 0.0.0.0/0 → :80, :8080 |
| ECS タスク用セキュリティグループ | ECS タスクへのアクセス制御 | インバウンド: ALB 用 SG → `container_port` のみ許可 |
