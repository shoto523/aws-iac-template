...
terraform init -backend-config="bucket=<YOUR_TFSTATE_BUCKET>" -backend-config="key=aws-app/terraform.tfstate" -backend-config="region=ap-northeast-1"
terraform plan
terraform apply
```
...
| パブリックサブネット（**異なるAZに2つ以上・必須**） | ALB・NAT Gateway の配置先 | ALB の仕様により異なるAZのサブネットが最低2つ必要。1つだとデプロイエラーになる |
| プライベートサブネット（1つ以上、複数AZ推奨） | ECS タスクの配置先 | 1つでも動作するが、可用性のため複数AZを推奨。NAT Gateway へのルートが必要 |
